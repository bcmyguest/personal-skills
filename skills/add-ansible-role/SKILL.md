...
Every new tool gets completions unless its binary genuinely can't generate them. The
repo keeps this in **one dedicated role** (`roles/shell-completions`) that regenerates
each script from the installed binary on every run — never a checked-in copy, which goes
stale silently. The new role installs the tool; the completions role learns about it via
a one-entry registry change in `vars/main.yml`.

The role auto-detects which shells exist on the machine (`command -v bash/zsh/fish`), so
there is no opt-in list to edit — describe the shells a generator supports and the role
writes only the ones actually installed.

**4a. Find the generator by running the real binary.** Don't guess the subcommand and
don't add an entry you haven't seen work. Probe, in this order, stopping at the first
that exits 0 and prints a script:

```bash
<tool> completion bash          # cobra (gh/glab use `completion -s bash`)
<tool> --completion bash        # or --completions bash
<tool> generate-shell-completion bash   # clap_complete / uv
<tool> --generate-shell-completion bash
<tool> --help | grep -i -e complet -e shell   # last resort: read the help text
```

Then confirm it's safe to install: output is non-empty, exits 0, and is **byte-stable**
across two runs (run it twice and `diff` — a script embedding a timestamp or a temp path
makes the role report changed forever). Check the generated script for helper binaries it
shells out to at completion time; each one belongs in `requires` alongside the tool
itself, or the user gets an error on every Tab press.

**The bash-completion 2.11 trap — check this before adding any bash entry.** grep the
generated bash script for `_comp_initialize` / `_comp_compgen`. Those are
bash-completion **2.12+** names; Ubuntu ships **2.11**, which only has `_init_completion`.
A script using them dies with `_comp_initialize: command not found` and returns zero
completions, on a box where everything looks correctly installed. Verify the machine:

```bash
grep -n "_comp_initialize\|_comp_compgen" <generated script>
bash -ic 'declare -F _comp_initialize >/dev/null && echo ok || echo "MISSING - inline the lib"'
```

The fix is to inline the library. Two routes, in order of preference:

- The tool has its own flag (mise): `mise completion bash --include-bash-completion-lib`.
- It doesn't, but it's a usage-cli tool (fnox — anything whose script says
  `@generated by usage-cli` and calls `usage complete-word`): go through the `usage` CLI
  instead, which has the flag generically. `--usage-cmd` keeps it pure argv, so it still
  fits a registry entry with no shell pipeline:

  ```bash
  usage generate completion bash <tool> --usage-cmd "<tool> usage" --include-bash-completion-lib
  ```

This costs ~125 KB per script and re-registers completions for unrelated builtins when
sourced — acceptable, because the alternative is broken. Do **not** use `lazy_argv` to
drop the flag on the autoload path: the 2.11 loader is present (so the probe passes) while
the 2.12 helpers are not, which is exactly the case that breaks. zsh and fish are
unaffected; only bash has this problem.

If nothing generates a script, **say so and record it** — add the tool to the "no
generator" comment in the registry's vars file so the next run of this skill doesn't
re-derive it. That's a complete outcome, not a failure.

**4b. Add the registry entry** to `shell_completions_tools`, matching the shape already
in the repo's vars file:

```yaml
  - name: <tool>
    requires: [<tool>]        # + any helper the generated script calls at Tab time
    shells: [bash, zsh, fish] # only shells whose generator you actually ran
    argv: [<tool>, completion, "{shell}"]
```

`{shell}` is substituted per shell. Split a tool into two entries when one shell needs a
different argv — that's why mise and fnox each have a bash entry and a zsh/fish entry.
Add `aliases: [<name>, ...]` for extra command names the script also completes; the role
installs them as links, which autoloading shells need since they key off the command
typed.

**4c. If the repo has no completions role**, create one by copying
`templates/shell-completions/` (tasks + vars) to `roles/shell-completions/`, then register
it in the playbook **last** — it reads the binaries the roles above it just installed:

```yaml
    # Last: it regenerates completions from the binaries installed above.
    - role: shell-completions
      tags: [shell-completions]
```

**Adding a new shell** is a vars-only change: add an entry to `shell_completions_shells`
with the drop-in `dir` and the `filename` pattern that shell expects (`{tool}` is
substituted), plus `lazy_dir`/`lazy_filename`/`lazy_probe` if the shell can autoload
scripts on demand. bash, zsh and fish ship pre-described. Don't add per-shell logic to
`tasks/main.yml`; if a shell seems to need it, that's a sign the shell description is
missing a field.
...
ansible-playbook <playbook> --tags <completions-role>   # after the tool exists on disk
```
...
pass must report no changes — same for the completions tag, whose drop-in file should now
exist (`ls ~/.bashrc.d/<tool>-completion.bash`) and stop reporting changed. If it reinstalls every time, the version comparison is
