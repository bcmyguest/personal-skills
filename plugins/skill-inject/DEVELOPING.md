...
Everything below runs from the crate dir:

```sh
cd plugins/skill-inject
```
...
- **Golden tests** (`tests/golden.rs`) discover this repo's actual
  `plugins/*/skills/**/SKILL.md` and assert the top-ranked skill for representative
  prompts (e.g. `"bootstrap a new python project with uv"` → `uv-setup`). They run on
  the offline bag-of-words backend so CI needs no model. When you add or rename a
  skill, update the golden cases; use `cargo run -- why "<prompt>"` to pick prompts
  and read the scores.
...
root, scoped to `plugins/skill-inject/**.rs`):
