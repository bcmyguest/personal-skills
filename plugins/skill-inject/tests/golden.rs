//! Golden tests: real prompts must rank the right repo skill first.
//!
//! Runs against this repo's actual `plugins/*/skills/**/SKILL.md` using the
//! offline bag-of-words embedder (default build), so it's network-free and
//! deterministic. With `--features fastembed` the same prompts should hold (and
//! gain synonym tolerance); add semantic-only prompts there later.
...
/// The repo's `plugins/` dir = the parent of this crate's manifest dir.
fn repo_plugins_root() -> PathBuf {
...
fn discovers_repo_skills() {
    let skills = skill::discover(&[repo_plugins_root()]).expect("discover");
